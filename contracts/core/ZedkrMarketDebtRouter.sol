...
interface IZedkrFactoryLike {
    function isMarket(address market) external view returns (bool);
}

interface IZedkrMarketLike {
...
contract ZedkrMarketDebtRouter is ReentrancyGuard {
    using SafeERC20 for IERC20;

    IZedkrFactoryLike public immutable factory;
    ...
    constructor(address factory_, address drp_) {
        ...
        factory = IZedkrFactoryLike(factory_);
        drp = IDRPLike(drp_);
    ...
    function depositForSelf(
        ...
        address collateral = IZedkrMarketLike(market).collateralAddress();
        if (collateral == address(0)) revert NativeUnsupported();

        IERC20(collateral).safeTransferFrom(msg.sender, address(this), amount);
        IERC20(collateral).forceApprove(market, amount);
        IZedkrMarketLike(market).deposit(outcomeIndex, amount, msg.sender, minSharesOut);
    ...
    function depositForSelfNative(
        ...
        IZedkrMarketLike(market).deposit{ value: msg.value }(
            outcomeIndex,
            msg.value,
            msg.sender,
            minSharesOut
    ...
    function redeemForSelf(
        ...
        address outToken = IZedkrMarketLike(market).outcomeToken(uint256(outcomeIndex));
        ...
        if (collateral == address(0)) {
            ...
            IZedkrMarketLike(market).redeem(outcomeIndex, shareAmount);
