...
import "../interfaces/IZedkrFeeReceiver.sol";

/// @title ZedkrFeeVault
/// @notice Epoch-based fee-sharing vault. Users stake the Zedkr governance token and receive
///         sZedkr receipt tokens. Trade fees are no longer split to stakers.
///
/// Fee flow:
...
///
/// Staking mechanics:
///   - Stake Zedkr → receive sZedkr 1:1. Each deposit gets its own unlock time.
...
contract ZedkrFeeVault is Ownable2Step, ReentrancyGuard, ERC165, IZedkrFeeReceiver {
    ...
    /// @notice The token users stake (Zedkr governance/utility token).
    ...
    /// @param stakeToken_    Zedkr governance token address.
    ...
    constructor(
        ...
        receiptToken = new sZedkrToken(address(this));
    }
    ...
    function supportsInterface(bytes4 interfaceId) public view override returns (bool) {
        return interfaceId == type(IZedkrFeeReceiver).interfaceId || super.supportsInterface(interfaceId);
    }
    ...
    /// @notice Stake `amount` of Zedkr tokens. Receive sZedkr 1:1.
    /// @param amount Amount of Zedkr to stake.
...
/// @title sZedkrToken
/// @notice Non-transferable ERC20 receipt token representing a staked Zedkr position.
///         Minted 1:1 on stake, burned on withdraw. Only the vault can mint/burn.
contract sZedkrToken is ERC20 {
    ...
    /// @dev Block all transfers — sZedkr is soul-bound to the staker.
