...
/**
 * Send ZedkrUSDC (deployment `contracts.ZedkrUSDC`; on-chain symbol is USDC) from PRIVATE_KEY → recipient (Base Sepolia).
 *
 * Env:
...
function readAftrAddress() {
  ...
  if (!addr || typeof addr !== "string") throw new Error("ZedkrUSDC missing in deployments/baseSepolia-84532.json");
  return addr;
}
