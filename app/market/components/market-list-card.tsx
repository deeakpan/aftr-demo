...
export function MarketListCard({
  ...
  return (
    <article
      className={`overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--card)] shadow-[var(--elevated-card-shadow)] ${className}`}
    >
      ...
      <div className="px-3 pb-2 pt-3">
        ...
        <div className="mt-2.5 flex max-h-[9.5rem] flex-col gap-1 overflow-y-auto pr-0.5 [scrollbar-width:thin] [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-[var(--border)]">
      ...
      <div className="flex items-center justify-between border-t border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-[11px] text-[var(--muted)]">
        {showVolume ? (
...
export function MarketListCardSkeleton({ className = "" }: { className?: string }) {
  return (
    <article
      className={`overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--card)] shadow-[var(--elevated-card-shadow)] ${className}`}
    >
      <div className={`${MARKET_COVER_ASPECT_CLASS} w-full animate-pulse bg-[var(--border)]/50`} />
      <div className="space-y-2 px-3 py-3">
        <div className="h-4 w-full animate-pulse rounded bg-[var(--border)]/50" />
        <div className="h-4 w-[70%] animate-pulse rounded bg-[var(--border)]/50" />
        {Array.from({ length: 3 }, (_, i) => (
          <div key={i} className="flex items-center gap-2 py-1">
            <div className="h-3 flex-1 animate-pulse rounded bg-[var(--border)]/50" />
            <div className="h-3 w-8 animate-pulse rounded bg-[var(--border)]/50" />
          </div>
        ))}
      </div>
