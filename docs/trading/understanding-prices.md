...
Zedkr Market uses a **parimutuel pool** model. There is no traditional order-book mid price for pool trades - instead, everyone’s collateral sits in shared pools per outcome, and shares are minted based on the pool’s current weights.
...
- Not a forecast from the platform - it is crowd-priced.
